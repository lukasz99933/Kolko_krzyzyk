...
Komputer::Komputer(Pole_do_gry& K, Wynik W)
  : pole(K)
  , wyn(W)
{
}
  ...
  for (int i = 0; i < pole.rozmiar; i++) // poruszamy się po macierzy
  {
    for (int j = 0; j < pole.rozmiar; j++) {
      if (pole.macierz[i][j] == ' ') {
        ...
        if (w > wynik) // sprawdzamy rezultat algorytmu , jeśli zachodzi warunek
                       // to ruch jest korzystny dla komputera
        {
          wynik = w;
          wsp1 = i;
          wsp2 = j;
...
int
Komputer::algorytm_minmax(char gracz)
{
  int wynik = -30;
  int w = -30;
  ...
  for (int i = 0; i < pole.rozmiar; i++) // poruszamy się po macierzy i
                                         // wypełniamy ją ruchami i sprawdzamy
                                         // dla kogo ruchy są korzystne
  {
    for (int j = 0; j < pole.rozmiar; j++) {
      if (pole.macierz[i][j] == ' ') {
