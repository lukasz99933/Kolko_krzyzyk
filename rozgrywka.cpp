...
Rozgrywka::Rozgrywka(Pole_do_gry &P, Wynik &W, Komputer &K, int wyb) : pole(P), wyn(W), komp(K), wybor(wyb)
...

{
    ...
    while(pole.ilosc_zapelnionych_pol<pole.rozmiar*pole.rozmiar && !wyn.czy_ktos_wygral()) //Wchodzimy do pętli gdy pole nie
...

    {
...

        {
            ...
            if(wspolrzedna_1<pole.rozmiar && wspolrzedna_2<pole.rozmiar && !czy_ruch_juz_taki_byl())
...

        {
            ...
            if(pole.ilosc_zapelnionych_pol%2==1 && !wyn.czy_ktos_wygral()) //Komputer
...
/*Funkcja sprawdza czy dany ruch już był, jeśli tak to dostajemy komunikat , zeby podać ponownie wspołrzędne  */

bool Rozgrywka::czy_ruch_juz_taki_byl()

{

    if(pole.macierz[wspolrzedna_1][wspolrzedna_2]=='o' || pole.macierz[wspolrzedna_1][wspolrzedna_2]=='x')
